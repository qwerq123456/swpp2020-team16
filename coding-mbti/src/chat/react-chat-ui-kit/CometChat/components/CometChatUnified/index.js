...
class CometChatUnified extends React.Component {
  ...
  componentDidMount() {
    ...
    new CometChatManager()
      ...
      .then((user) => {
        ...
        console.log(this.loggedInUser); //여기서 유저 못받음
      })
      .catch((error) => {
  ...
  groupUpdated = (message, key, group, options) => {
    switch (key) {
      ...
      case enums.GROUP_MEMBER_SCOPE_CHANGED: {
        if (options.user.uid === this.loggedInUser.uid) {
          const newObj = Object.assign({}, this.state.item, {
            scope: options["scope"],
          });
          this.setState({
            item: newObj,
  ...
  render() {
    ...
    if (this.state.threadmessageview) {
      threadMessageView = (
        <div
          css={unifiedSecondaryStyle(this.theme)}
          className="unified__secondary-view"
        >
          <MessageThread
            theme={this.theme}
            tab={this.state.tab}
    ...
    return (
      <div
        css={unifiedStyle(this.theme)}
        className="cometchat cometchat--unified"
      >
        <div
          css={unifiedSidebarStyle(this.state, this.theme)}
        ...
        {detailScreen}
        {threadMessageView}
        <CallAlert theme={this.theme} actionGenerated={this.actionHandler} />
        <CallScreen
          theme={this.theme}
          item={this.state.item}
          type={this.state.type}
